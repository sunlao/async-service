...
from shared.models.xform import ModelResult
...
class XFormJobResult(BaseModel):
    model_config = DTO_CONFIG
    RunId: str
    Status: bool
    Results: Optional[tuple[ModelResult, ...]] = None
    Command: str
    Duration: float
    LockAquired: bool
...
class XFormEvent(BaseModel):
    model_config = DTO_CONFIG
    JobId: int = Field(gt=0)
    JobName: str
    Message: str
    Status: bool
    Results: Optional[XFormJobResult] = None
    Start: datetime
    End: datetime
    DurationMs: int = Field(ge=0)

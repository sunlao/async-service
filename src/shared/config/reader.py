...
from shared.models.worker import AdminConfig, JobConfig, MovementConfig, XFormConfig
...
class Reader:
    ...
    def _job_config(self, config, job_type: JobTypes) -> JobConfig:
        ...
        if job_type == JobTypes.MOVEMENT:
            ...
            return dto
        if job_type == JobTypes.XFORM:
            dto: JobConfig[XFormConfig] = JobConfig(
                Type=job_type,
                Config=self._xform(config),
                KWARGS=config.get("kwargs", {}),
            )
            return dto
    ...
    def _xform(self, config) -> XFormConfig:
        return XFormConfig(
            Id=config["id"],
            Name=config["name"],
            ActionType=config["action_type"],

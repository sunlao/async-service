...
from worker.handler.xform import XForm as XFHandler
...
async def xform(ctx, config: JobConfig):
    """Routes all xform jobs to it's Handler"""

    result = await XFHandler(ctx).handle(config.Config, **config.KWARGS)
    if result.Event.Status is False and ctx["job_try"] < config.Config.Retry:
        raise Retry(defer=timedelta(minutes=1))
    return result
...
class AppSettings:
    functions = [admin, flush, movement, xform]
    keep_result = 14400

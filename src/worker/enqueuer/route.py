...
class Route:
    ...
    async def _enqueue(
        ...
        if job_type in (JobTypes.ADMIN, JobTypes.XFORM):
            return await self.control.enqueue(re_enqueue, job_type, enqueue_config)
    ...
    async def _run_next(self, job_config: JobConfig):
        ...
        if job_type in (JobTypes.ADMIN, JobTypes.XFORM):
            await self.control.enqueue(False, job_type, job_config)
        if job_type == JobTypes.MOVEMENT:

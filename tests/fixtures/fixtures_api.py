...
@asynccontextmanager
async def _app_state(engine, arq_client, config_log, config_aserv):
    app = FastAPI()
    s = app.state
    ...
    s.app_version = config_aserv.AppVersion
...
async def f_api_with_state(engine, arq_client, config_log, config_aserv):
    async with _app_state(engine, arq_client, config_log, config_aserv) as app:
        yield app

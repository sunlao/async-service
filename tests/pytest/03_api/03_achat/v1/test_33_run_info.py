...
async def test_detail(aserv_api_client, api_request, reader):
    ...
    async with timeout(300):
        while status != "complete":
            await sleep(2)
            response = await aserv_api_client.get(f"runs/{run_id1}")
            doc = response.json()
            status = doc["Status"]
    ...
    async with timeout(300):
        while status != "complete":
            await sleep(2)
            response = await aserv_api_client.get(f"runs/{run_id2}")
            doc = response.json()
            status = doc["Status"]
    assert status == "complete"
    response = await aserv_api_client.post("enqueue/102")
    assert response.status_code == 202
    doc = response.json()
    run_id3 = doc["RunID"]
    ...
    async with timeout(300):
        while status != "complete":
            await sleep(2)
            response = await aserv_api_client.get(f"runs/{run_id3}")
            doc = response.json()
            status = doc["Status"]
    assert status == "complete"

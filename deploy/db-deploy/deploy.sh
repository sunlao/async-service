...
function init_database {
    ...
    -c "CREATE SCHEMA IF NOT EXISTS stage;" \
    -c "CREATE SCHEMA IF NOT EXISTS working;"
    ...
    -c "DO \$\$
BEGIN
IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = '$DB_DATA_USER') THEN
    EXECUTE format('CREATE ROLE %I LOGIN PASSWORD %L', '$DB_DATA_USER', '$DB_DATA_PWD');
ELSE
    EXECUTE format('ALTER ROLE %I PASSWORD %L', '$DB_DATA_USER', '$DB_DATA_PWD');
END IF;
END
\$\$;" \
    -c "DO \$\$
...
\$\$;" 
    ...
    -c "GRANT USAGE, CREATE ON SCHEMA raw TO $DB_DATA_USER;" \
    -c "GRANT USAGE, CREATE ON SCHEMA stage TO $DB_DATA_USER;" \
    -c "GRANT USAGE, CREATE ON SCHEMA working TO $DB_DATA_USER;" \
    -c "ALTER DEFAULT PRIVILEGES IN SCHEMA $APP_SCHEMA GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON TABLES TO $DB_DATA_USER;" \
    -c "ALTER DEFAULT PRIVILEGES IN SCHEMA raw GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON TABLES TO $DB_DATA_USER;" \
    -c "ALTER DEFAULT PRIVILEGES IN SCHEMA stage GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON TABLES TO $DB_DATA_USER;" \
    -c "ALTER DEFAULT PRIVILEGES IN SCHEMA working GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON TABLES TO $DB_DATA_USER;" \
    ...
    -c "GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON ALL TABLES IN SCHEMA raw TO $DB_DATA_USER;" \
    -c "GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON ALL TABLES IN SCHEMA stage TO $DB_DATA_USER;" \
    -c "GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON ALL TABLES IN SCHEMA working TO $DB_DATA_USER;" \
    ...
    -c "REVOKE ALL ON SCHEMA stage, working FROM $DB_APP_USER;" \
    ...
    -c "GRANT USAGE ON SCHEMA raw TO $DB_APP_USER;" \
    -c "ALTER DEFAULT PRIVILEGES IN SCHEMA raw GRANT SELECT ON TABLES TO $DB_APP_USER;" \
    -c "ALTER DEFAULT PRIVILEGES IN SCHEMA raw GRANT EXECUTE ON FUNCTIONS TO $DB_APP_USER;" \
    -c "GRANT SELECT ON ALL TABLES IN SCHEMA raw TO $DB_APP_USER;"
}
